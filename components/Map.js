...
export default function Map({ loading, setLoading }) {
  ...
  useEffect(async () => {
    // make the loading screen appear if the api is still loading
    setLoading(true);
    try {
      ...
      // if API has no problems, remove the loading component
      setLoading(false);
    } catch (error) {
      console.log('Error: ', error);
      // if API has problems, remove the loading component
      setLoading(false);
    }
  }, []);
